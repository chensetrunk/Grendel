...
public:
    ...
    __aicore__ inline void Init(
        GM_ADDR orig_points_gm_in, // orig_points (means3D) : float3* effectively, so float*
        GM_ADDR scales_gm_in,    // scales : float3*
        GM_ADDR rotations_gm_in,  // rotations : float4*
        GM_ADDR opacities_gm_in,   // opacities : float*
        GM_ADDR shs_gm_in,         // shs : float* ( (D+1)^2 * 3 coefficients per gaussian)
        GM_ADDR clamped_gm_out,      // clamped : bool* (output)
        GM_ADDR cov3D_precomp_gm_in, // (optional) precomputed 3D covariances
        GM_ADDR colors_precomp_gm_in,// (optional) precomputed colors
        GM_ADDR viewmatrix_gm_in,  // float[16]
        GM_ADDR projmatrix_gm_in,  // float[16]
        GM_ADDR cam_pos_gm_in,     // float[3]
        GM_ADDR radii_gm_out, // radii : int* (output)
        GM_ADDR points_xy_image_gm_out, // points_xy_image : float2* (output)
        GM_ADDR depths_gm_out,      // depths : float* (output)
        GM_ADDR cov3Ds_gm_out,  // cov3Ds : float* (output, if not precomputed)
        GM_ADDR rgb_gm_out,     // rgb : float* (output, if not precomputed)
        GM_ADDR conic_opacity_gm_out, // conic_opacity : float4* (output)
        ...
        orig_points_gm.SetGlobalBuffer((__gm__ float*)orig_points_in_gm + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        scales_gm.SetGlobalBuffer((__gm__ float*)scales_in_gm + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        rotations_gm.SetGlobalBuffer((__gm__ float*)rotations_in_gm + globalBufferOffsetP * 4, this->numGaussiansPerCore * 4);
        opacities_gm.SetGlobalBuffer((__gm__ float*)opacities_in_gm + globalBufferOffsetP, this->numGaussiansPerCore);
        // SHs are (M_sh * 3) floats per Gaussian. M_sh = (D_sh+1)^2
        uint32_t M_sh = (tiling.D + 1) * (tiling.D + 1);
        shs_gm.SetGlobalBuffer((__gm__ float*)shs_in_gm + globalBufferOffsetP * M_sh * 3, this->numGaussiansPerCore * M_sh * 3);
        ...
        viewmatrix_gm_const.SetGlobalBuffer((__gm__ float*)viewmatrix_in_gm, 16);
        projmatrix_gm_const.SetGlobalBuffer((__gm__ float*)projmatrix_in_gm, 16);
        cam_pos_gm_const.SetGlobalBuffer((__gm__ float*)cam_pos_in_gm, 3);

        // Initialize GlobalTensor objects (outputs)
        clamped_gm.SetGlobalBuffer((__gm__ bool*)clamped_gm_out + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        radii_gm.SetGlobalBuffer((__gm__ int*)radii_gm_out + globalBufferOffsetP, this->numGaussiansPerCore);
        points_xy_image_gm.SetGlobalBuffer((__gm__ float*)points_xy_image_gm_out + globalBufferOffsetP * 2, this->numGaussiansPerCore * 2);
        depths_gm.SetGlobalBuffer((__gm__ float*)depths_gm_out + globalBufferOffsetP, this->numGaussiansPerCore);
        cov3Ds_gm.SetGlobalBuffer((__gm__ float*)cov3Ds_gm_out + globalBufferOffsetP * 6, this->numGaussiansPerCore * 6);
        rgb_gm.SetGlobalBuffer((__gm__ float*)rgb_gm_out + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        conic_opacity_gm.SetGlobalBuffer((__gm__ float*)conic_opacity_gm_out + globalBufferOffsetP * 4, this->numGaussiansPerCore * 4);
...
private:
    ...
    __aicore__ inline void CopyOut(int32_t loopIdx) {
        ...
        AscendC::DataCopy(cov3Ds_gm[gm_offset * 6], cov3Ds_out_local, this->currentLoopDataNum * 6);
        AscendC::DataCopy(rgb_gm[gm_offset * 3], rgb_out_local, this->currentLoopDataNum * 3);
...
private:
    ...
    AscendC::GlobalTensor<float> cov3Ds_gm;
    AscendC::GlobalTensor<float> rgb_gm;
...
extern "C" __global__ __aicore__ void npu_preprocess_forward(
    ...
    if (TILING_KEY_IS(1)) { // Indicates big cores exist
        ...
        op.Init(orig_points_gm_in, scales_gm_in, rotations_gm_in, opacities_gm_in, shs_gm_in, clamped_gm_out,
                cov3D_precomp_gm_in, colors_precomp_gm_in, viewviewmatrix_gm_inmatrix_gm, projmatrix_gm_in, cam_pos_gm_in,
                radii_gm_out, points_xy_image_gm_out, depths_gm_out, cov3Ds_gm_out, rgb_gm_out, conic_opacity_gm_out,
...
public:
    ...
    __aicore__ inline void Init(
        ...
        this->points_xy_image_gm.SetGlobalBuffer((__gm__ float*)points_xy_image_gm_in); // Full array
        this->features_gm.SetGlobalBuffer((__gm__ float*)features_gm_in);             // Full array
        this->conic_opacity_gm.SetGlobalBuffer((__gm__ float*)conic_opacity_gm_in);   // Full array

        // Input: Ranges defining Gaussians per tile
        this->ranges_gm.SetGlobalBuffer((__gm__ uint32_t*)ranges_gm_in); // Assuming ranges are (uint32_t start, uint32_t end) pairs, so effectively uint32_t*

        // Input: Sorted list of Gaussian indices
        this->point_list_gm.SetGlobalBuffer((__gm__ uint32_t*)point_list_gm_in);

        // Input: Background color (small, load once)
        this->bg_color_gm.SetGlobalBuffer((__gm__ float*)bg_color_gm_in, NPU_NUM_CHANNELS);

        // Optional: compute_locally map (if not all tiles are processed by this core/rank)
        // this->compute_locally_gm.SetGlobalBuffer((__gm__ bool*)compute_locally_gm_in);
        ...
        this->final_T_gm.SetGlobalBuffer((__gm__ float*)final_T_gm_out);
        this->n_contrib_gm.SetGlobalBuffer((__gm__ uint32_t*)n_contrib_gm_out);
        this->out_color_gm.SetGlobalBuffer((__gm__ float*)out_color_gm_out);
...
public:
    ...
    __aicore__ inline void Init(
        ...
        this->ranges_gm.SetGlobalBuffer((__gm__ uint32_t*)ranges_gm_in);
        this->point_list_gm.SetGlobalBuffer((__gm__ uint32_t*)point_list_gm_in);
        this->points_xy_image_gm.SetGlobalBuffer((__gm__ float*)points_xy_image_gm_in);
        this->conic_opacity_gm.SetGlobalBuffer((__gm__ float*)conic_opacity_gm_in);
        this->colors_gm.SetGlobalBuffer((__gm__ float*)colors_gm_in);
        this->final_Ts_gm.SetGlobalBuffer((__gm__ float*)final_Ts_gm_in);
        this->n_contrib_gm.SetGlobalBuffer((__gm__ uint32_t*)n_contrib_gm_in);
        this->bg_color_gm.SetGlobalBuffer((__gm__ float*)bg_color_gm_in, NPU_NUM_CHANNELS);
        this->dL_dpixels_gm.SetGlobalBuffer((__gm__ float*)dL_dpixels_gm_in); // Full image gradient H*W*C
        ...
        this->dL_dmean2D_gm.SetGlobalBuffer((__gm__ float*)dL_dmean2D_gm_out); // Size P * 2 (for float2)
        this->dL_dconic2D_gm.SetGlobalBuffer((__gm__ float*)dL_dconic2D_gm_out); // Size P * 3 (for float3 part of conic)
        this->dL_dopacity_gm.SetGlobalBuffer((__gm__ float*)dL_dopacity_gm_out); // Size P
        this->dL_dcolors_gm.SetGlobalBuffer((__gm__ float*)dL_dcolors_gm_out); // Size P * NUM_CHANNELS
...
public:
    ...
    __aicore__ inline void Init(
        ...
        // Inputs
        means3D_gm.SetGlobalBuffer((__gm__ float*)means3D_gm_in + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        radii_gm.SetGlobalBuffer((__gm__ int*)radii_gm_in + globalBufferOffsetP, this->numGaussiansPerCore);
        cov3Ds_fwd_gm.SetGlobalBuffer((__gm__ float*)cov3Ds_fwd_gm_in + globalBufferOffsetP * 6, this->numGaussiansPerCore * 6);
        dL_dconics_gm.SetGlobalBuffer((__gm__ float*)dL_dconics_gm_in + globalBufferOffsetP * 4, this->numGaussiansPerCore * 4); // dL_dconic_opacity

        viewmatrix_gm_const.SetGlobalBuffer((__gm__ float*)viewmatrix_gm_in, 16);

        // Outputs
        dL_dmean3D_cov_gm.SetGlobalBuffer((__gm__ float*)dL_dmean3D_cov_gm_out + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        dL_dcov3D_gm.SetGlobalBuffer((__gm__ float*)dL_dcov3D_gm_out + globalBufferOffsetP * 6, this->numGaussiansPerCore * 6);
...
public:
    ...
    __aicore__ inline void Init(
        ...
        // Inputs
        means3D_gm.SetGlobalBuffer((__gm__ float*)means3D_gm_in + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        scales_gm.SetGlobalBuffer((__gm__ float*)scales_gm_in + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        rotations_gm.SetGlobalBuffer((__gm__ float*)rotations_gm_in + globalBufferOffsetP * 4, this->numGaussiansPerCore * 4);
        shs_fwd_gm.SetGlobalBuffer((__gm__ float*)shs_fwd_gm_in + globalBufferOffsetP * M_sh * 3, this->numGaussiansPerCore * M_sh * 3);
        radii_fwd_gm.SetGlobalBuffer((__gm__ int*)radii_fwd_gm_in + globalBufferOffsetP, this->numGaussiansPerCore);
        clamped_fwd_gm.SetGlobalBuffer((__gm__ bool*)clamped_fwd_gm_in + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        dL_dmean2D_render_gm.SetGlobalBuffer((__gm__ float*)dL_dmean2D_render_gm_in + globalBufferOffsetP * 2, this->numGaussiansPerCore * 2);
        dL_dmean3D_cov_gm.SetGlobalBuffer((__gm__ float*)dL_dmean3D_cov_gm_in + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        dL_dcolors_render_gm.SetGlobalBuffer((__gm__ float*)dL_dcolors_render_gm_in + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        dL_dcov3D_cov_gm.SetGlobalBuffer((__gm__ float*)dL_dcov3D_cov_gm_in + globalBufferOffsetP * 6, this->numGaussiansPerCore * 6);

        projmatrix_gm_const.SetGlobalBuffer((__gm__ float*)projmatrix_gm_in, 16);
        cam_pos_gm_const.SetGlobalBuffer((__gm__ float*)cam_pos_gm_in, 3);

        // Outputs
        dL_dmeans3D_total_gm.SetGlobalBuffer((__gm__ float*)dL_dmeans3D_total_gm_out + globalBufferOffsetP*3, this->numGaussiansPerCore*3);
        dL_dshs_gm.SetGlobalBuffer((__gm__ float*)dL_dshs_gm_out + globalBufferOffsetP * M_sh * 3, this->numGaussiansPerCore * M_sh * 3);
        dL_dscales_gm.SetGlobalBuffer((__gm__ float*)dL_dscales_gm_out + globalBufferOffsetP * 3, this->numGaussiansPerCore * 3);
        dL_drots_gm.SetGlobalBuffer((__gm__ float*)dL_drots_gm_out + globalBufferOffsetP * 4, this->numGaussiansPerCore * 4);
