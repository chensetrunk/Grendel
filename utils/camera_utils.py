...
def loadCam(args, id, cam_info, decompressed_image=None, return_image=False):
    ...
    utils.set_img_size(target_h, target_w)
    
    # Instead of assertion, we'll use the target size
    if orig_w != target_w or orig_h != target_h:
        utils.print_rank_0(f"Warning: Image {cam_info.image_path} size ({orig_w}, {orig_h}) differs from target size ({target_w}, {target_h}). Resizing...")
    
    resolution = (target_w, target_h)  # Make sure resolution is a tuple
