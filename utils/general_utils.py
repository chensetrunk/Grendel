...
def get_img_width():
    """Get the target image width for the dataset"""
    args = get_args()
    return args.resolution_w if hasattr(args, 'resolution_w') else 1920


def get_img_height():
    """Get the target image height for the dataset"""
    args = get_args()
    return args.resolution_h if hasattr(args, 'resolution_h') else 1080
