...
def final_system_loss_computation(
    ...
    target_w = utils.get_img_width()
    target_h = utils.get_img_height()
    
    if image.shape[2] != target_w or image.shape[1] != target_h:
        utils.print_rank_0(f"Warning: Rendered image size {image.shape[2]}x{image.shape[1]} differs from target size {target_w}x{target_h}. Resizing...")
        image = torch.nn.functional.interpolate(image.unsqueeze(0), size=(target_h, target_w), mode='bilinear', align_corners=False).squeeze(0)
    ...
    # Ensure GT image dimensions match
    if local_image_rect_gt.shape[2] != target_w or local_image_rect_gt.shape[1] != (coverage_max_y - coverage_min_y):
        utils.print_rank_0(f"Warning: GT image size {local_image_rect_gt.shape[2]}x{local_image_rect_gt.shape[1]} differs from expected size {target_w}x{coverage_max_y - coverage_min_y}. Resizing...")
        local_image_rect_gt = torch.nn.functional.interpolate(local_image_rect_gt.unsqueeze(0), size=(coverage_max_y - coverage_min_y, target_w), mode='bilinear', align_corners=False).squeeze(0)
